...
export function TheMovies({data,show}){
    ...
    if (data.Response == 'False') return (<div>Filme não encontrato</div>)
...
export default function Movies33(){
    ...
    const {data, error} = useSWR(state.url, async () => {
        ...
        const res = await fetch(`${state.url}/?apikey=8ba47b1d&s=${state.titleSearchString}`)
    ...
    return (
        <div>
            <TheForm filtro={'Buscar Titulo'} url={state.url} acao={onClickHandler}/>
